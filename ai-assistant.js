...
/**
 * Build the complete financial context string for AI prompts.
 */
function _buildFullFinanceContext() {
  ...
  const totalIncome = _sum(txns, "income");
  const totalExpense = _sum(txns, "expense");
  ...
  const allCats = { ...curCats };
  Object.keys(prevCats).forEach((c) => {
    if (!allCats[c]) allCats[c] = 0;
  });

  const catLines = Object.keys(allCats)
    .sort((a, b) => (allCats[b] || 0) - (allCats[a] || 0))
    .map((c) => {
      const cur = curCats[c] || 0;
      const prev = prevCats[c] || 0;
      const chg =
        prev > 0
          ? ` (${cur > prev ? "+" : ""}${Math.round(((cur - prev) / prev) * 100)}% vs last month)`
          : "";
      return `  ${c}: ₹${Math.round(cur)}${chg}`;
    })
    .join("\n");

  const recentLines = txns
    .slice(0, 30)
    .map(
      (t) =>
        `  ${t.date} | ${t.type} | ${t.category} | ${t.description || "-"} | ${t.type === "income" ? "+" : "-"}₹${Math.round(Math.abs(t.amount))}`,
    )
    .join("\n");
  ...
  const dowLines = Object.entries(dowMap)
    .map(([d, v]) => `  ${dowNames[d]}: ₹${Math.round(v)}`)
    .join("\n");

  return `=== BlueLedger Financial Context ===
Date: ${now.toLocaleDateString("en-IN", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}
Day ${now.getDate()} of ${daysInMonth} | ${daysLeft} days left this month

--- This Month ---
Income:  ₹${Math.round(tmIncome)}
Expense: ₹${Math.round(tmExpense)}
Net:     ₹${Math.round(tmIncome - tmExpense)}
Avg daily spend: ₹${avgDaily}
Projected month-end spend: ₹${projected}
${limit > 0 ? `Monthly budget limit: ₹${limit} (${Math.round((tmExpense / limit) * 100)}% used)` : "No budget limit set"}

--- Last Month ---
Expense: ₹${Math.round(lmExpense)}

--- All Time ---
Total income:  ₹${Math.round(totalIncome)}
Total expense: ₹${Math.round(totalExpense)}
Total transactions: ${txns.length}

--- Category Breakdown (this month) ---
${catLines || "  No expenses yet"}

--- Spending by Day of Week (this month) ---
${dowLines}
...
function computeHealthScore() {
  ...
  if (tmIncome > 0) {
    ...
    if (savingsRate >= 0.3) {
      ...
      factors.push({
        type: "positive",
        text: `Strong savings rate: ${Math.round(savingsRate * 100)}% of income saved`,
      });
    } else if (savingsRate >= 0.1) {
      score += 10;
      factors.push({
        type: "neutral",
        text: `Moderate savings rate: ${Math.round(savingsRate * 100)}%`,
      });
      ...
      factors.push({
        type: "neutral",
        text: `Low savings rate: ${Math.round(savingsRate * 100)}%`,
      });
    }
  }
  ...
  if (lmExpense > 0 && tmExpense > 0) {
    ...
    if (trend < -0.1) {
      ...
      factors.push({
        type: "positive",
        text: `Spending down ${Math.round(-trend * 100)}% vs last month`,
      });
    } else if (trend > 0.25) {
      score -= 10;
      factors.push({
        type: "negative",
        text: `Spending up ${Math.round(trend * 100)}% vs last month`,
      });
    }
  }
  ...
  if (limit > 0) {
    ...
    if (used <= 0.8) {
      ...
      factors.push({
        type: "positive",
        text: `Well within monthly budget (${Math.round(used * 100)}% used)`,
      });
    } else if (used <= 1.0) {
      score += 0;
      factors.push({
        type: "neutral",
        text: `Approaching budget limit (${Math.round(used * 100)}% used)`,
      });
    } else {
      score -= 15;
      factors.push({
        type: "negative",
        text: `Over budget — ${Math.round((used - 1) * 100)}% exceeded`,
      });
    }
  }
  ...
  if (txns.length >= 10) {
    score += 5;
    factors.push({ type: "positive", text: "Consistent financial tracking" });
  }
  ...
  if (hasInvestment) {
    ...
    factors.push({
      type: "positive",
      text: "Active investment activity detected",
    });
  }
...
function generateLocalInsights() {
  ...
  if (!txns.length)
    return [
      { icon: "💡", text: "Add your first transaction to unlock AI insights." },
    ];
  ...
  if (tmIncome > 0) {
    ...
    if (tmExpense > tmIncome) {
      insights.push({
        icon: "⚠️",
        text: `You're spending more than you earn this month — ₹${Math.round(tmExpense - tmIncome)} over budget.`,
        type: "danger",
      });
    } else {
      insights.push({
        icon: "💰",
        text: `You're saving ${rate}% of your income this month (₹${Math.round(tmIncome - tmExpense)}).`,
        type: "positive",
      });
    }
  }
  ...
  if (fastest && fastest.pct >= 15) {
    insights.push({
      icon: "🔥",
      text: `${fastest.category} spending grew ${fastest.pct}% vs last month.`,
      type: "warn",
    });
  }
  ...
  if (topCat) {
    insights.push({
      icon: "🏆",
      text: `${topCat[0]} is your biggest spend this month at ₹${Math.round(topCat[1])}.`,
      type: "info",
    });
  }
  ...
  if (avgDaily > 0) {
    insights.push({
      icon: "📅",
      text: `Your average daily spend is ₹${Math.round(avgDaily)} this month.`,
      type: "info",
    });
  }
  ...
  if (projected > 0) {
    if (limit > 0 && projected > limit) {
      ...
      insights.push({
        icon: "🚨",
        text: `At this pace, you'll spend ₹${projected.toLocaleString("en-IN")} — ₹${(projected - limit).toLocaleString("en-IN")} over your ₹${limit.toLocaleString("en-IN")} budget.`,
        type: "danger",
      });
    } else {
      insights.push({
        icon: "🔮",
        text: `Projected month-end spend: ₹${projected.toLocaleString("en-IN")}.`,
        type: "info",
      });
    }
  }
  ...
  if (txnCount > 5 && weekendTotal > weekdayTotal * 0.6) {
    insights.push({
      icon: "🎉",
      text: "Weekend spending is significantly higher than weekdays — leisure patterns detected.",
      type: "warn",
    });
  }
  ...
  if (maxDow && maxDow[1] > 0) {
    ...
    insights.push({
      icon: "📆",
      text: `${dayNames[maxDow[0]]} is your highest spending day this month.`,
      type: "info",
    });
  }
...
function generateBudgetRecs() {
  ...
  if (!recs.length) {
    recs.push({
      ...
      body: "Your spending patterns look healthy. Keep maintaining this balance.",
    });
  }
...
function openAIAssistant() {
  ...
  if (!_aiaScoreLoaded) {
    _renderHealthScore();
    _aiaScoreLoaded = true;
  }
  if (!_aiaInsightsLoaded) {
    _renderInsightCards();
    _renderBudgetRecs();
    _aiaInsightsLoaded = false;
  } // always refresh
...
async function _aiaRunDeepInsights() {
  ...
  btn.disabled = true;
  btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> AI is thinking…';
  panel.style.display = "block";
  panel.innerHTML = `<div class="aia-deep-loading"><i class="fas fa-brain" style="color:#a78bfa"></i><span>Running deep financial analysis…</span></div>`;

  try {
    const system = `You are BlueLedger AI, an expert personal finance advisor for an Indian user.
Analyse the financial data below and provide a deep, actionable report.

Return ONLY valid JSON (no markdown) in this exact shape:
{
  "headline": "One punchy headline summary",
  "overallAssessment": "2-3 sentence overall assessment",
  "topInsights": [{"title":"string","body":"string","severity":"low|medium|high"}],
  "predictions": [{"label":"string","value":"string","reasoning":"string"}],
  "actionPlan": [{"step":"string","impact":"string"}]
}

Rules:
- topInsights: 3-5 specific, data-driven insights
- predictions: 2-3 forward-looking predictions with specific amounts
- actionPlan: 3-4 concrete, prioritised steps
- Use ₹ for amounts. Return ONLY the JSON.`;

    const raw = await _callAI(
      [{ role: "user", content: _buildFullFinanceContext() }],
      system,
      900,
    );
    ...
    const insightsHtml = (parsed.topInsights || [])
      .map(
        (i) => `
    ...
    const predsHtml = (parsed.predictions || [])
      .map(
        (p) => `
    ...
    const stepsHtml = (parsed.actionPlan || [])
      .map(
        (s, i) => `
      <div class="aia-deep-step">
        <div class="aia-deep-step-num">${i + 1}</div>
        <div>
    ...
    panel.innerHTML = `
      <div class="aia-deep-result">
        ...
        <div class="aia-deep-section-title" style="margin-top:1rem"><i class="fas fa-crystal-ball" style="color:#818cf8"></i> Predictions</div>
    ...
    panel.innerHTML = `<div class="aia-deep-error"><i class="fas fa-circle-exclamation" style="color:#ef4444"></i> AI analysis failed. Please try again.</div>`;
    console.warn("AIA deep insights failed:", e);
  } finally {
    btn.disabled = false;
    btn.innerHTML =
      '<i class="fas fa-wand-magic-sparkles"></i> Run AI Deep Analysis';
...
async function _aiaSendChat() {
  ...
  _appendChatMsg(
    "assistant",
    `<span id="${typingId}" class="aia-typing"><span></span><span></span><span></span></span>`,
  );
  ...
  try {
    const system = `You are BlueLedger AI, a warm, concise personal finance assistant for an Indian user.
Answer the user's question using ONLY the financial data provided. 
Be specific with numbers, use ₹ for amounts, keep responses under 150 words.
If data is insufficient, say so honestly. Never invent transactions.
Use emojis sparingly. Format with line breaks for readability.
...
${_buildFullFinanceContext()}`;
    ...
    if (_aiaChatHistory.length > 20)
      _aiaChatHistory = _aiaChatHistory.slice(-20);
