...
function _extractWordAmount(transcript) {
  const patterns = [
    ...
    new RegExp(
      `\\b(?:spent|paid|used|received|earned|got|made|income(?:\\s+of)?|expense(?:\\s+of)?|salary(?:\\s+of)?)\\s+((?:(?:${VOICE_NUMBER_WORD_PATTERN}|hundred|thousand|lakh|lac|point|and)\\s+){0,10}(?:${VOICE_NUMBER_WORD_PATTERN}|hundred|thousand|lakh|lac))\\b`,
      "i",
    ),
  ];
...
function _extractVoiceAmount(transcript) {
  const patterns = [
    ...
    /\b(?:spent|paid|used|received|earned|got|made|income(?:\s+of)?|expense(?:\s+of)?|salary(?:\s+of)?)\s+(\d+(?:,\d+)*(?:\.\d+)?)\b/i,
  ];
...
function _cleanVoiceDescription(type, transcript, matchedText) {
  ...
  const actionPattern =
    type === "income"
      ? /\b(?:please\s+)?(?:add|log|record|note)?\s*(?:i\s+)?(?:just\s+)?(?:received|earned|got|made|credited|credit|income(?:\s+from)?|salary(?:\s+from)?|freelance(?:\s+payment)?|bonus(?:\s+from)?|refund(?:\s+from)?)\b/gi
      : /\b(?:please\s+)?(?:add|log|record|note)?\s*(?:i\s+)?(?:just\s+)?(?:spent|paid|used|bought|ordered|booked|gave|purchased|had|expense(?:\s+for)?|charged?(?:\s+for)?)\b/gi;
