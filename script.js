...
function renderCategoryManager() {
  ...
  if (customCategories.length === 0) {
    list.innerHTML = '<div class="cat-empty-state"><i class="fas fa-tag"></i><span>No custom categories yet</span></div>';
    return;
  }
    ...
    .map(
      ...
      <div class="custom-cat-item">
        <div class="custom-cat-info">
          <span class="custom-cat-dot" style="background:${getCatColor(c)}"></span>
          <span class="custom-cat-name">${c}</span>
        </div>
        <button class="btn-cat-remove" onclick="removeCustomCategory(${i})" title="Remove ${c}">
